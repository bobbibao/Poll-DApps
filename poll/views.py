    ...
    if request.method == "POST":
        ...
        try:
            ...
            if profile is not None:
                ...
                user.profile_pic = "img/default-avatar.png"   
            user.save()
...
def poll_detail(request, poll_id):
    ...
    if poll.active is False:
        if mobile(request):
            return render(request, 'home/result_mobile.html', {'poll': poll})
        return render(request, 'home/result_pc.html', {'poll': poll})
...
class result(View):
    def get(self, request, poll_id):
        ...
        return render(request, 'home/result_pc.html', {
            'poll': poll,
        })
...
def view(request):
    ...
    if 'search' in request.GET:
        ...
        all_polls = all_polls.filter(text__icontains=search_term)
    so_pag = 8
    paginator = Paginator(all_polls, so_pag)
    page = request.GET.get('page')
    polls = paginator.get_page(page)

    get_dict_copy = request.GET.copy()
    params = get_dict_copy.pop('page', True) and get_dict_copy.urlencode()
    print(ceil(paginator.count/so_pag))
    pag = []
    for i in range(1,ceil(paginator.count/so_pag)+1):
        pag.append(i)
    context = {
        'pag': pag,
        'polls': polls,
